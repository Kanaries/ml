...
description: Explore Gaussian, Multinomial, Complement, Bernoulli, and Categorical Naive Bayes in JavaScript and TypeScript with @kanaries/ml.
---
...
- [GaussianNB](gaussianNB): best for continuous numeric features
- [MultinomialNB](multinomialNB): best for non-negative count features
- [ComplementNB](complementNB): count-feature classifier that can help with imbalanced classes
...
1. Use [GaussianNB](gaussianNB) when features are continuous numeric measurements.
2. Use [MultinomialNB](multinomialNB) or [ComplementNB](complementNB) when features are non-negative counts.
3. Use [BernoulliNB](bernoulliNB) when features represent yes/no states, token presence, clicks, or flags.
4. Use [CategoricalNB](categoricalNB) when each feature is a discrete category.
